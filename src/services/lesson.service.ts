...
export class LessonService {
  ...
  private lessonsUrl = 'http://learn.it-akademy.fr:8000/api/v1/lesson';  // URL to web api
  headers;
  ...
  rateLesson(id: number, value: number): Promise<string> {
    ...
    return this.http
      .post('http://learn.it-akademy.fr:8000/api/v1/ratings', {rateable_id: id, value: value, rateable_type: 1}, {headers: this.headers})
      .toPromise()
      .then(res => res.json().data)
      .catch(this.handleError);
  }
  ...
  getCategories(): Promise<Category[]> {
    return this.http.get('http://learn.it-akademy.fr:8000/api/v1/category', {headers: this.headers})
               .toPromise()
  ...
  getLesByCat(idCategory: number): Promise<Lesson[]> {
    return this.http.get(`${'http://learn.it-akademy.fr:8000/api/v1/lesByCat'}/${idCategory}`, {headers: this.headers})
               .toPromise()
               .then(response => response.json().data as Lesson[])
               .catch(this.handleError);
  }
  ...
  getRatingLesson(idLesson: number): Promise<Ratings>{
    return this.http.get(`${'http://learn.it-akademy.fr:8000/api/v1'}/1/ratings/${idLesson}`, {headers: this.headers})
               .toPromise()
               .then(response => response.json().data as Ratings)
               .catch(this.handleError);
  }
  getRatingProblematic(idProblematic: number): Promise<void>{
    return this.http.get(`${'http://learn.it-akademy.fr:8000/api/v1'}/2/ratings/${idProblematic}`, {headers: this.headers})
               .toPromise()
